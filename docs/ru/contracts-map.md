...
Цель: зафиксировать живую поверхность IPC, preload и renderer-контрактов Burrow.
...
`instances:list` возвращает метаданные каждого инстанса вместе со сводкой из одного канонического снимка. Это позволяет спискам не запрашивать состояние каждого инстанса отдельно.
...
`window.api.network` содержит три отдельные capability. Burrow Link, LAN discovery и UPnP отдают независимые типизированные lifecycle-снимки и подписки; изменяемого network mode в main-процессе больше нет. Нативные ошибки, данные роутера и идентификаторы пиров не пересекают preload-границу.

В Burrow Link `active` означает работающую локальную службу, а не подтверждённую игровую сессию. `peerCount` показывает доступных пиров; `metrics.activeGameConnectionCount` — открытые локальные игровые потоки, тогда как `gameConnectionCount` остаётся накопительным счётчиком. Таймаут игрового подключения возвращает безопасный `TUNNEL_PEER_UNAVAILABLE`, сохраняя локальный адрес для повторной попытки. Renderer не применяет снимки с меньшей ревизией поверх новых событий.
...
`settings:openMinecraftPath` не принимает путь из renderer. Главный процесс открывает только сохранённую папку, ранее выбранную нативным диалогом, либо свою папку по умолчанию. Перед открытием проверяется, что путь локальный и ведёт к существующей директории; UNC и device paths отклоняются.

Резервная копия настроек принимает только явно перечисленные и ограниченные по размеру настройки лаунчера и последние варианты запуска. Нативные диалоги, атомарная запись, проверка схемы и лимит размера импорта остаются в main-процессе. Аккаунты, токены, идентификатор аналитики, приглашения Burrow Link, локальные пути и игровые файлы исключены.
...
- `providerCatalog:contents` — временно скачивает только архив выбранной удалённой версии и возвращает её манифест и список override-файлов; модпак и перечисленные в манифесте файлы не устанавливаются.
