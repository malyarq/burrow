...
describe('DownloadsTab layout', () => {
  it('keeps embedded tuning controls inside the shared settings shell at laptop widths', () => {
    ...
    const inputsRow = screen.getByDisplayValue('8').closest('.border-t') as HTMLElement;
    ...
    expect(tuningShell.className).toContain('settings-section-shell');
    expect(tuningShell.className).not.toContain('surface-muted');
    ...
    expect(inputsRow.className).toContain('border-t');
