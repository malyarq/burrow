...
- [x] Блокировать Windows/macOS release без signing credentials.
- [ ] Настроить реальные сертификаты, Apple notarization и проверку подписи установленных артефактов.
...
Релиз допустим только когда проходят `npm ci`, `npm run verify`, production build на трёх ОС, macOS visual closeout, ограниченный full-install smoke и установленное обновление с предыдущей версии. Production audit не должен содержать high или critical findings. Windows/macOS artifacts должны быть подписаны, macOS — notarized. Любой невыполненный критерий означает prerelease, а не stable.
