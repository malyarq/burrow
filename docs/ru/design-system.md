...
Здесь описан поддерживаемый UI-контракт Burrow Next (`1.0.0-next.1`). Каноническая реализация находится в `src/index.css`, `src/product/`, `src/contexts/settings/` и `src/components/ui/`.
...
Публичное название продукта — **Burrow**, а функция совместной игры называется **Burrow Link**. Русский слоган: **Играй локально. Зови друга.** В оболочке Next используются новый знак, иллюстрации и иконки. Они поддерживают навигацию и содержание страницы, а не становятся ещё одной интерактивной панелью.

| Роль | Значение |
| --- | --- |
| Тёмное поле знака | `#252529` |
| Светлый дверной проём | `#f4f1e9` |
| Тёплая деталь проёма | `#d5a66b` |

Канонический файловый знак — `public/burrow-symbol.svg`: геометрический дверной проём на тёмном скруглённом поле. `src/product/BurrowSymbol.tsx` — его inline-вариант для интерфейса; он использует `currentColor`, поэтому получает цвет от окружающего элемента. `public/burrow-next-landscape.png` — сгенерированная иллюстрация для оболочки Next. Файлы в `docs/assets/brand/` — исторические материалы и не являются источником актуального знака или иконок. Не добавляйте вокруг знака отдельную декоративную карточку; wordmark и знак остаются самостоятельными элементами навигации.
...
Приложение поддерживает нейтральные light и dark mode. Пользователь выбирает акцент отдельно от режима и может сохранить свои цвета. Presets определены в `src/contexts/settings/theme-presets.ts`:
...
Смена preset должна сохранять явный override пользователя и показывать понятную цель reset. Не добавляйте локальный theme state внутрь компонента. Движение должно быть неброским; оно учитывает `prefers-reduced-motion` и класс `disable-animations`.

### Оболочка Next

У продукта четыре верхнеуровневые страницы: «Играть», «Библиотека», «Вместе» и «Настройки». Верхняя навигация выбирает одну страницу, а не открывает конкурирующие панели. «Настройки» — полноценная страница; после первого открытия она остаётся смонтированной при переходе, чтобы не терять данные формы и положение прокрутки.
...
Эти классы владеют radius, border, opacity, backdrop и shadows. Не собирайте тот же surface заново локальными классами route. В оболочке Next используйте простые page sections и не вкладывайте декоративные карточки без самостоятельного содержания или действия.
