...
- [x] Fail Windows/macOS publishing when signing credentials are absent.
- [ ] Configure real certificates, Apple notarization and installed-artifact signature checks.
...
Stable `v0.7.0` requires `npm ci`, `npm run verify`, builds on all three OSes, macOS visual closeout, bounded full-install smoke and installed update from the previous version. Production audit must have no high or critical findings. Windows/macOS must be signed and macOS notarized. Otherwise the artifact is a prerelease.
