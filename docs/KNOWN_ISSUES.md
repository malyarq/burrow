...
| --- | --- | --- | --- |
| High | macOS and Windows artifacts are not trustworthy without code signing; macOS also needs notarization. | Release CI fails closed when signing credentials are absent and publishes only after all platform builds finish. | Configure `MACOS_CSC_*`, Apple notarization and `WINDOWS_CSC_*` secrets; verify signatures on installed artifacts. |
