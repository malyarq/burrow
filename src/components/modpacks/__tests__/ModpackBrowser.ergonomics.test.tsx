...
import catalogSource from '../browser/catalogAnalytics.ts?raw';
...
describe('ModpackBrowser ergonomics', () => {
  ...
  it('keeps the route as composition with focused query and presentation owners', () => {
    ...
    expect(controllerSource).toContain('searchCatalog');
    expect(controllerSource).toContain('loadCatalogVersions');
    expect(controllerSource).not.toContain('providerCatalogIPC');
    expect(catalogSource).toContain('providerCatalogIPC');
