...
Проверка обновления должна загрузить предыдущий пакет с тестовыми настройками, статистикой и выбранным непустым инстансом, затем проверить их сохранность новым пакетом. Одного постороннего файла-маркера недостаточно. На Windows запускайте установочные проверки в одноразовой VM или чистом runner: NSIS меняет регистрацию установки и ярлыки, поэтому временная папка сама по себе не изолирует рабочую установку пользователя.
