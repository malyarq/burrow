...
export function MultiplayerConnectionControls({ multiplayer }: { multiplayer: ReturnType<typeof useMultiplayer> }) {
  ...
  const tunnelStatusKey = (multiplayer.tunnel.metrics?.activeGameConnectionCount ?? 0) > 0
    ? 'multiplayer.game_stream_open'
    : multiplayer.tunnel.peerCount > 0 ? 'multiplayer.peer_ready' : 'multiplayer.endpoint_waiting';
  ...
  const liveStatus = status || (active ? t(isTunnel ? tunnelStatusKey : isLan ? 'multiplayer.lan_broadcast_active' : 'multiplayer.upnp_mapping_active') : '');

  const renderHost = () => <div className="space-y-5">
    {!active && <>
    ...
    {active && isTunnel && multiplayer.roomCode && <div className={cn('multiplayer-room-code space-y-4 rounded-2xl border p-4 sm:p-5', getAccentStyles('soft-bg').className, getAccentStyles('soft-border').className)}>
      <div>
        <span className="mb-2 block text-xs font-medium text-secondary">{t('multiplayer.room_active')}</span>
        <span className="block break-all font-mono text-base font-semibold tracking-[0.12em] text-foreground sm:text-lg">{formatBurrowLinkCode(multiplayer.roomCode)}</span>
      </div>
  ...
  const renderJoin = () => {
    if (isTunnel) return multiplayer.mappedPort ? <div className="space-y-5">
      <StateCard title={t(tunnelStatusKey)} detail={multiplayer.directAddress} />
    ...
    return multiplayer.lan.state !== 'active' ? <Button className="w-full" onClick={multiplayer.join} isLoading={multiplayer.isLoading}>{t('multiplayer.scan_lan')}</Button> : <div className="space-y-5"><div className="max-h-52 space-y-2 overflow-y-auto" aria-live="polite">{multiplayer.discovered.length === 0 && <p className="py-8 text-center text-sm text-secondary">{t('multiplayer.no_lan_servers')}</p>}{multiplayer.discovered.map((server) => <button key={`${server.address}:${server.port}`} type="button" onClick={() => multiplayer.selectLanServer(server)} className="surface-soft w-full rounded-xl border border-transparent p-3.5 text-left transition-colors hover:border-border"><span className="block font-medium">{server.motd}</span><span className="mt-1 block font-mono text-xs text-secondary">{server.address}:{server.port}</span></button>)}</div><StopButton onStop={multiplayer.stop} label={t('multiplayer.stop')} /></div>;
  };

  return <>
    {!isUpnp && <div className="surface-soft grid grid-cols-2 rounded-xl p-1" role="tablist" aria-label={t('multiplayer.title')}>{(['host', 'join'] as const).map((mode) => <button type="button" key={mode} onClick={() => multiplayer.setMode(mode)} id={`multiplayer-mode-tab-${mode}`} role="tab" aria-selected={multiplayer.mode === mode} aria-controls={modePanelId} className="multiplayer-mode-tab normal-case" style={{ outlineColor: '#fff' }}>{t(`multiplayer.${mode}`)}</button>)}</div>}
    <div id={modePanelId} role={isUpnp ? undefined : 'tabpanel'} aria-labelledby={isUpnp ? undefined : `multiplayer-mode-tab-${multiplayer.mode}`} className="space-y-5">
      <p className="border-b border-border/60 pb-4 text-sm leading-6 whitespace-pre-line text-secondary">{t(isTunnel ? (multiplayer.mode === 'host' ? 'multiplayer.host_desc' : 'multiplayer.join_desc') : isLan ? 'multiplayer.lan_desc' : 'multiplayer.upnp_desc')}</p>
...
function StateCard({ title, detail }: { title: string; detail: string }) {
  return <div className="surface-soft rounded-2xl border border-border/80 p-4 text-center sm:p-5"><p className="mb-2 text-xs font-medium text-secondary">{title}</p><p className="break-all font-mono text-sm font-semibold text-foreground">{detail}</p></div>;
}
